...
def weights_agg(weights_list, device):
    agg_weights = copy.deepcopy(weights_list[0])
    for key in agg_weights.keys():
        agg_weights[key] = agg_weights[key]
        
    for i in range(1, len(weights_list)):
        for key in agg_weights.keys():
            agg_weights[key] += weights_list[i][key]
    
    for key in agg_weights.keys():
        agg_weights[key] = agg_weights[key] / len(weights_list)
...
def parse_arguments():
    ...
    parser.add_argument('--log_filename', type=str, default=f"./Log/log_fed_noCommu_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
    parser.add_argument('--raw_data_filename', type=str, default="NUMO_raw6")
    parser.add_argument('--processed_data_filename', type=str, default="fed")
    parser.add_argument('--history_filename', type=str, default=f"Fed_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
...
if __name__=='__main__':
    ...
    for i in range(num_clients):
        ...
        if isDCNN:
            ...
            #LSTM
            LSTMEncoder = LSTMModel(in_channels, 80, depth, out_channels, 1)
            regressor = LinearRegressor(out_channels, output_dim)
    ...
    for t in range(num_rounds):
        ...
        # Aggregate combined model weights
        agg_weights = weights_agg([local_models[i].state_dict() for i in range(num_clients)], mDevice)
    ...
    # # check model size
    # for i in range(num_clients):
    #     torch.save(local_models[i].state_dict(), f"{log_filename}_{i}_centralized_best_model.pth")
    #     model_size = os.path.getsize(f"{log_filename}_{i}_centralized_best_model.pth")
