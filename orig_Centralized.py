...
from NUMO_load_data import load_numo_dataset, smoothData, refactorState
...
def parse_arguments():
    ...
    parser.add_argument('--remove_fea_index', type=list_of_ints, default=[]) #time
    ...
    parser.add_argument('--isAttention', type=int, default=0)
...
if __name__=='__main__':
                        ...
                        for out_channels in out_channels_lists:
                            for kernel_size in kernel_size_lists:
                                for learning_rate in learning_rates_lists:
                                    ...
                                    local_models, history = fit_model_hyperparameters(local_models, central_train, central_train_labels, None, None, initial_weights, cuda, gpu=device_id, num_epochs=num_epochs, learning_rate=learning_rate, batch_size=batch_size, scheduler=None, filename=history_filename, client_id=0)
    ...
    # # check model size
    # torch.save(best_model, f"{log_filename}_centralized_best_model.pth")
    # model_size = os.path.getsize(f"{log_filename}_centralized_best_model.pth")
